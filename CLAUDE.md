...
Both `CatalogCacheService` and `BusinessLinesCatalogCacheService` source from the same adapter: `OracleBusinessLinesCatalogSourceAdapter` (implements both `CatalogSourcePort` and `BusinessLinesCatalogSourcePort`). It runs the SQL files in `src/main/resources/sql/business-lines/oracle/`. The POS presentation flags are loaded from `IN_OMNI_BUSINESS_LINE_ITEM` in TRX3 through `item-business-line-config.sql` and merged with the RMS catalog by `rms_item_code`. Active when `app.business-lines.source=oracle` (the default). Fallbacks: `InMemoryCatalogSourceAdapter` and `InMemoryBusinessLinesCatalogSourceAdapter` when `source=memory`.
